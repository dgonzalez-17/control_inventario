<template>
    <div class="signUp_user">
        <div class="container_Logistic">
            <h2>Crear logistica</h2>
            <form v-on:submit.prevent="processCreateLogistic">
...
export default {
    ...
    methods: {
        processCreateLogistic: function () {
            axios.post(
                "http://127.0.0.1:8000/logistica/crear",{
                    name: this.user.name,
                ...
                .then((result) => {
                   ...
                   if (resultado.message == "datos validados") {
                    let createLogistic = this.user.name 
                    alert("Creación exitosa de logistica" + createLogistic);
                    location.reload(this.$router.push({ name: "formCreateLogistic" }));
