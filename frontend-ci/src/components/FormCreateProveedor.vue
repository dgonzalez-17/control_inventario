<template>
    <div class="createProv">
        <div class="container_createCustomer">
            <h2>Crear Proveedor</h2>
            <form v-on:submit.prevent="processCreateCustomer">
...
export default {
    ...
    methods: {
        processCreateCustomer: function () {
            axios.post(
                "http://127.0.0.1:8000/proveedor/crear",{
                    name: this.user.name,
                ...
                .then((result) => {
                   ...
                   if (resultado.message == "datos validados") {
                    let createCustomer = this.user.name 
                    alert("Creación exitosa del cliente", createCustomer);
                    location.reload(this.$router.push({ name: "formCreateCustomer" }));
